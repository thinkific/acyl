...
func TestNewTrackingRef(t *testing.T) {
	hook, rc, ctrl := newTestGitHubEventWebhook(t, testSecret, testTypePath)
	defer ctrl.Finish()
	rc.EXPECT().GetFileContents(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(testYAMLTrackRefsBytes, nil)
	newDummyPushGithubEvent(t)
	b, _ := newDummyPushGithubEvent(t)
	sig := hook.GenerateSignatureString(b)
	out, err := hook.New(b, sig)
	if err != nil {
		t.Fatalf("should have succeeded: %v", err)
	}
	if out.Action != Update {
		t.Fatalf("expected Update, got: %v", out.Action.String())
	}
}

func TestNewTrackingRefNotRelevant(t *testing.T) {
	hook, rc, ctrl := newTestGitHubEventWebhook(t, testSecret, testTypePath)
	defer ctrl.Finish()
	rc.EXPECT().GetFileContents(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(testYAMLTrackRefsBytes, nil)
	_, event := newDummyPushGithubEvent(t)
	event.Ref = "heads/release"
	...
	if out.Action != NotRelevant {
		t.Fatalf("expected NotRelevant, got: %v", out.Action.String())
	}
}

func TestNewTrackingRefNoTrackingRefs(t *testing.T) {
	hook, rc, ctrl := newTestGitHubEventWebhook(t, testSecret, testTypePath)
	defer ctrl.Finish()
	rc.EXPECT().GetFileContents(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(testYAMLMasterTargetBytes, nil)
	b, _ := newDummyPushGithubEvent(t)
