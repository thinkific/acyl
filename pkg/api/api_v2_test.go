...
func TestExternalDocumentURLsHandler(t *testing.T) {
	dl, tdl := testdatalayer.New(testlogger, t)
	if err := tdl.Setup(testDataPath); err != nil {
		t.Fatalf("error setting up test database: %v", err)
	}
	...
	docUrls := []string{"https://foo.com", "https://bar.com/baz"}
	apiv2, err := newV2API(dl, nil, nil, config.ServerConfig{APIKeys: []string{"foo"}, UIExternalDocumentURLs: docUrls}, OAuthConfig{}, testlogger, nil)
	if err != nil {
		t.Fatalf("error creating api: %v", err)
	}
	...
	req, _ := http.NewRequest("GET", "https://foo.com/v2/help/document/urls", nil)
	req = req.Clone(withSession(req.Context(), uis))

	rc := httptest.NewRecorder()

	apiv2.externalDocumentURLsHandler(rc, req)
	...
	out := V2ExternalDocumentURLs{}
	if err := json.NewDecoder(res.Body).Decode(&out); err != nil {
		t.Fatalf("error decoding response: %v", err)
	}
	res.Body.Close()

	// check expected total urls
	if len(out.DocumentURLs) != 2 {
		t.Fatalf("expected 2, got %v", len(out.DocumentURLs))
	}
	// check url order is maintained
	if out.DocumentURLs[1] != docUrls[1] {
		t.Fatalf("expected %v, got %v", docUrls[1], out.DocumentURLs[1])
	}
}
