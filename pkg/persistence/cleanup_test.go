...
func TestCleanupPruneDestroyedEnvRecords(t *testing.T) {
	...
	if err := dl.CreateQAEnvironment(&models.QAEnvironment{Created: time.Now().UTC().Add(-1 * time.Hour), Name: "foo-bar"}); err != nil {
		t.Fatalf("environment create should have succeeded: %v", err)
	}
	if err := dl.SetQAEnvironmentStatus("foo-bar", models.Destroyed); err != nil {
		t.Fatalf("environment set status should have succeeded: %v", err)
	}
	expires := 1 * time.Millisecond
	time.Sleep(expires * 2)
	if err := dl.CreateQAEnvironment(&models.QAEnvironment{Created: time.Now().UTC(), Name: "foo-bar-2"}); err != nil {
	...
	dl.CreateK8sEnv(&models.KubernetesEnvironment{EnvName: "foo-bar"})
	dl.CreateHelmReleasesForEnv([]models.HelmRelease{models.HelmRelease{EnvName: "foo-bar"}})
	dl.SetQAEnvironmentCreated("foo-bar-2", time.Now().UTC().Add(1*time.Hour))
	if err := c.PruneDestroyedEnvRecords(expires); err != nil {
		t.Fatalf("prune should have succeeded: %v", err)
	}
	env, err := dl.GetQAEnvironment("foo-bar")
	if err != nil {
	...
	k8senv, err := dl.GetK8sEnv("foo-bar")
	if err != nil {
	...
	hr, err := dl.GetHelmReleasesForEnv("foo-bar")
	if err != nil {
	...
	env, err = dl.GetQAEnvironment("foo-bar-2")
	if err != nil {
