...
func (ci ChartInstaller) Cleanup(ctx context.Context, objMaxAge, deletionDelay time.Duration, dryRun bool) {
	if err := ci.removeOrphanedNamespaces(ctx, objMaxAge, deletionDelay, dryRun); err != nil {
		ci.log(ctx, "error cleaning up orphaned namespaces: %v", err)
	}
	if err := ci.removeOrphanedCRBs(ctx, objMaxAge, deletionDelay, dryRun); err != nil {
		ci.log(ctx, "error cleaning up orphaned ClusterRoleBindings: %v", err)
	}
}

// removeOrphanedNamespaces removes orphaned namespaces
func (ci ChartInstaller) removeOrphanedNamespaces(ctx context.Context, maxAge, deletionDelay time.Duration, dryRun bool) error {
	if maxAge == 0 {
...
func (ci ChartInstaller) removeOrphanedCRBs(ctx context.Context, maxAge, deletionDelay time.Duration, dryRun bool) error {
	if maxAge == 0 {
		return errors.New("maxAge must be greater than zero")
	}
